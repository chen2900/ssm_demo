...
<body>
	...
	<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;left: 30px;">
		<div class="btn-group" style="position: relative; top: 18%;">
		  <button type="button" class="btn btn-primary" onclick="window.location.href='/settings/dictionary/value/save.html'"><span class="glyphicon glyphicon-plus"></span> 新增</button>
		...
		<table class="table table-hover table-striped">
			...
			<c:forEach items="${values}" var="v" varStatus="sta">
				<tr draggable="true" ><%--data-type="${v.typeId}"--%>
					...
					<td>${v.value}</td>
					<td>${v.text}</td>
					<td><input name="orderNo" data-id="${v.id}" value="${v.orderNo}" style="width:50px;border:none;background-color: transparent;outline: none;" /></td>
					<%--<td>${v.orderNo}</td>--%>
					<td>${v.type.name}</td>
				</tr>
			</c:forEach>
		...
		<div class="row">
			...
			<div class="col-md-6" style="margin-left: 150px">
				<nav aria-label="Page navigation">
					<ul class="pagination">
						<li><a href="/value/index.html?pn=1" rel="external nofollow" >首页</a></li>
						<c:if test="${pageInfo.hasPreviousPage }">
							<li>
								<a href="/value/index.html?pn=${pageInfo.pageNum-1}" rel="external nofollow" aria-label="Previous">
						...
						<c:forEach items="${pageInfo.navigatepageNums}" var="page_Num">
							...
							<c:if test="${page_Num != pageInfo.pageNum }">
								<li><a href="/value/index.html?pn=${page_Num}" rel="external nofollow" >${page_Num}</a></li>
							</c:if>
						</c:forEach>
						<c:if test="${pageInfo.hasNextPage }">
							   <li>
								   <a href="/value/index.html?pn=${pageInfo.pageNum+1}" rel="external nofollow" aria-label="Next">
								   		<span aria-hidden="true" >下一页</span>
								   </a>
							   </li>
						</c:if>
						<li><a href="/value/index.html?pn=${pageInfo.pages}" rel="external nofollow" >末页</a></li>
					</ul>
				</nav>
			</div>
		</div>
